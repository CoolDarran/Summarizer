...
public class CWordDBSCAN {
	...
	public void Cluster(List<CWord> words,double epsilon,int minPts,String POS){
...
//		long beginTime = System.currentTimeMillis();
		for(int index=0;index<wordList.size();++index){
			...
			if(p.isClassed()||p.getSense()==-1||p.getLemma().length()<2)
//			if(p.isClassed())
				continue;
...
//			System.out.println(p.getLemma() + "," + q.getLemma());
			if(p.getLemma().equals(q.getLemma()) 
					|| WordSimilarity.simWord(p.getLemma(),p.getSense(),q.getLemma(),q.getSense())>=e){
				++count;
