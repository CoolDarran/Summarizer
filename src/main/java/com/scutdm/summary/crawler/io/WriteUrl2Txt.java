...
public class WriteUrl2Txt {
    ...
    static {
        if (CrawlProperties.contains("crawl.storage.result")) {
            resDir = CrawlProperties.getProperty("crawl.storage.result");
        } else {
            resDir = "crawl_result/";
        }
    ...
    public void write(String url, String urlResult) {
        ...
        try {
            ...
            generatedName = generateFileName(url);
        ...
        File file = new File(resDir + generatedName + ".txt");
        file.deleteOnExit();
        PrintWriter pw = null;
        try {
